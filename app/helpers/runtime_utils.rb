module RuntimeUtils
	...
	def parse(command_line_info)
		...
		format_info = format_info(info) # associate the correct keys with the command_line_info
		check_validity(format_info)
	end

	def format_info(parsed_info)
		information.shift #remove unnecessary first element - i.e. 'add' or 'credit'
		formatted_hash = Hash.new
		if information.length == 3 # that is, if command_prompt started with'add'
			CARD_CREATION_INFO.each do |element|
				formatted_hash[element] = information.shift
			end
		else
	...
	def check_validity(formatted_info)
		unless is_limit_valid?(formatted_info[-1]) # both limit and amount need to be in the correct format
	...
	def is_limit_valid?(limit)
		limit[0] == '$' && limit[1..-1].to_i != 0 # the first character of the limit needs to be a $, the rest needs to be a non 0 number
	end
